    ...
    <ul className="max-h-[75vh] flex flex-col gap-2 p-2 items-stretch overflow-y-scroll">
      { games.map(game => {
        return <li className="bg-zinc-900 rounded p-2 pl-4 flex items-center justify-between" style={getCSSTints(isColourDark(game.target))}>
          <div>
            <h3 className="font-bold sm:text-xl whitespace-nowrap text-center">{ new Date(game.date).toLocaleDateString() }</h3>
          ...
          <div className="inline-flex justify-center bg-tint/10 p-2">
            { game.guesses.map(guess => <div style={{ background: `#${guess}` }} className="w-8 h-8 sm:w-12 sm:h-12"></div>) }
            <svg xmlns="http://www.w3.org/2000/svg" viewBox='0 0 48 48' className="w-8 h-8 sm:w-12 sm:h-12 fill-zinc-600">
            ...
            <div style={{ background: `#${game.target}` }} className="w-8 h-8 sm:w-12 sm:h-12"></div>
          </div>
        </li>
      }) }
    </ul>
